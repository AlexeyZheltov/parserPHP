...
*******************************************************************************/
  ...
  if (!empty($_GET['site_go']))
  {
	$id_site = $_GET['site_go'];
  ...
  $query = "SELECT * FROM parser_site_list  WHERE stop_scan = 1 AND block = 0  LIMIT " . $page * 1000 . ", 1000"; //
  $result = mysqlQuery($query);
  $i = 0;
  while ($res_site = mysql_fetch_assoc($result))
  {	$site_array[$i] = $res_site;  // массив с данными сайтов
	...
	// Проходим по всем сайтам
	$query = "SELECT COUNT(*) FROM parser_site_list WHERE stop_scan = 1 AND block = 0 ";
	$result = mysqlQuery($query);
