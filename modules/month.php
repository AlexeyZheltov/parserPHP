...
*******************************************************************************/
  ...
  $query = "SELECT * FROM parser_site_list  WHERE  ((UNIX_TIMESTAMP() - UNIX_TIMESTAMP(last_update)) < 30*60*60*24 ) AND block = 0 AND last_update > 0 LIMIT " . $page * 1000 . ", 1000"; //
  $result = mysqlQuery($query);
  $i = 0;
  ...
  while ($res_site = mysql_fetch_assoc($result))
  {	$site_array[$i] = $res_site;  // массив с данными сайтов
	...
	// Проходим по всем сайтам
	$query = "SELECT COUNT(*) FROM parser_site_list WHERE  ((UNIX_TIMESTAMP() - UNIX_TIMESTAMP(last_update)) < 30*60*60*24 ) AND block = 0 AND last_update > 0";
	$result = mysqlQuery($query);
